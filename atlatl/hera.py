            ...
            for bars in rights:
                for bar in bars.bars:
                    t2.writelines(["%s %s\n" %
                                   ...
                                   for _ in range(bar.multiplicity)])
                t2.write("--\n")
        dists = subprocess.check_output(["bottleneck_dist", t1_name, t2_name, str(relative_error)])
...
def array_bottleneck_distance(lefts,
    ...
    if len(lefts.shape) != 3:
        raise ValueError("`lefts` must have shape (# of barcodes, # of bars in each code, 3)")
    if len(rights.shape) != 3:
        raise ValueError("`rights` must have shape (# of barcodes, # of bars in each code, 3)")
    if lefts.shape[0] != rights.shape[0]:
        raise ValueError("First dimension of both arrays must have the same length")
    with tempfile.TemporaryDirectory() as temp:
        t1_name = os.path.join(temp, 'self.txt')
        t2_name = os.path.join(temp, 'other.txt')
        with open(t1_name, 'wt') as t1:
            for code in range(lefts.shape[0]):
                for bar in range(lefts.shape[1]):
                    ...
                    if np.isnan(one_bar[2]):
                        continue
                    t1.writelines(["%s %s\n" % (one_bar[0], min(inf, one_bar[1]))] * int(one_bar[2]))
                t1.write("--\n")
        with open(t2_name, 'wt') as t2:
