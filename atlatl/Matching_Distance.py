...
def matching_distance(Mod1, Mod2, Grid_Parameter, Normalize, Fixed_Bounds):
    ...
    # Input:
    # Mod1,Mod2: RIVET "precomputed" representations of two persistence
    # modules, in Bryn's python bytes format

    # Grid_Parameter: This is a non-negative integer which should be at least 1.
    # We will choose Grid_Parameter values of slope and also choose
    # Grid_Parameter offset values, for each slope.
    ...
    # uniform precision over a large collection of 2-D persistence modules,
    # which may exhibit features at different scales.

    # TODO: In the future Fixed_Bounds should be an optional agrument, instead
    # of this empty list nonsense.
    ...
    for i in range(Grid_Parameter):
        ...
        # find the offset parameters such that the lines with slope sl just
        ...
        # interest.
            ...
            for j in range(Grid_Parameter):
                offset = LR_Offset + j * \
                    (UL_Offset - LR_Offset) / (Grid_Parameter - 1)
                # print("offset", offset)
    ...
    for i in range(len(List_Of_Lines)):
        ...
        if Normalize:
            ...
            # of course, this code can be made more compact, but hopfully this
            # way is readible
